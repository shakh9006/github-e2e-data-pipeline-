...
with DAG(
    ...
    drop_stg_table_before = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
            DROP TABLE IF EXISTS {STG_SCHEMA}.tmp_fct_top_users;
        """,
    )
    ...
    create_stg_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        CREATE TABLE IF NOT EXISTS {STG_SCHEMA}.tmp_fct_top_users AS
        SELECT 
	    owner_login AS login, 
    ...
    create_target_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        CREATE TABLE IF NOT EXISTS {DM_SCHEMA}.fct_top_users (
    ...
    update_target_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        DELETE FROM {DM_SCHEMA}.fct_top_users;
        INSERT INTO {DM_SCHEMA}.fct_top_users
        SELECT 
            *
        FROM {STG_SCHEMA}.tmp_fct_top_users;
        """
    )
