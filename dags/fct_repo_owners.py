...
with DAG(
    ...
    drop_stg_table_before = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
            DROP TABLE IF EXISTS {STG_SCHEMA}.tmp_fct_repo_owners;
        """,
    )
    ...
    create_stg_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        CREATE TABLE IF NOT EXISTS {STG_SCHEMA}.tmp_fct_repo_owners AS
        SELECT 
            owner_type,
            COUNT(*) AS count
    ...
    create_target_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        CREATE TABLE IF NOT EXISTS {DM_SCHEMA}.fct_repo_owners (
            owner_type VARCHAR(255) PRIMARY KEY,
            count INT
        );
        """
    )
    ...
    update_target_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        DELETE FROM {DM_SCHEMA}.fct_repo_owners;
        INSERT INTO {DM_SCHEMA}.fct_repo_owners
        SELECT 
            owner_type,
            count
        FROM {STG_SCHEMA}.tmp_fct_repo_owners;
        """
    )
